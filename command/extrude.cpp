...
void Extrude::go()
{
  ...
  if (containers.empty())
  {
    observer->outBlocked(msg::buildStatusMessage("Wrong pre selection for extrude", 2.0));
    shouldUpdate = false;
    return;
  }
  ...
  if (!fId)
  {
    observer->outBlocked(msg::buildStatusMessage("No feature id for extrude", 2.0));
    shouldUpdate = false;
    return;
  }
