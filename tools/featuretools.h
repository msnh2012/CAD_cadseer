...
namespace tls
{
  ...
  struct Resolved
  {
    Resolved(const ftr::Base *, const boost::uuids::uuid&, const ftr::Pick&); //[[deprecated("no id")]]
    Resolved(const ftr::Base *, const ftr::Pick&);
    Resolved(const ftr::Pick&);
    const ftr::Base *feature = nullptr; //!< feature containing the result
    boost::uuids::uuid resultId; //!< result id of pick in feature. [[deprecated("use Pick::resolvedIds")]]
    ...
    occt::ShapeVector getShapes() const;
  };
  ...
  /*! @brief Version 2 of pick resolution
   * 
   * @param pick will be copied into return with resolved ids cleared and then filled in.
   * @param payload input information generated by project.
   * @return Resolved object with feature pointer and with pick containing resolvedIds.
   * @note highlightIds of pick will not be modified. pick.tag needs to match UpdatePayload
   * tags in order for resolution to happen.
   */
  Resolved
  resolvePick
  (
