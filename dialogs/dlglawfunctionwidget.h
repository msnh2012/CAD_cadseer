...
#ifndef DLG_LAWFUNCTIONWIDGET_H
#define DLG_LAWFUNCTIONWIDGET_H
...
//     void smooth();
  ...
  /*! @class LawFunctionDialog
   * @brief Modal dialog for editing law functions
   * 
   */
  class LawFunctionDialog : public QDialog
  {
    Q_OBJECT
  public:
    LawFunctionDialog();
    LawFunctionDialog(QWidget*);
    ~LawFunctionDialog() override;
    void setCue(const lwf::Cue &cueIn){lawFunctionWidget->setCue(cueIn);}
    const lwf::Cue& getCue(){return lawFunctionWidget->getCue();}
    bool hasStructureChanged(){return lawFunctionWidget->hasStructureChanged();}
  private:
    void buildGui();
